Common Table Expressions (CTEs) for High-Value Customers generally starts with with keyword
...
--  Categorize customers based on their total loan amount.
 
SELECT c.customer_id, c.name, SUM(l.loan_amount) AS total_loan,
       CASE 
           WHEN SUM(l.loan_amount) > 20000 THEN 'High Value'
           WHEN SUM(l.loan_amount) BETWEEN 10000 AND 20000 THEN 'Medium Value'
           ELSE 'Low Value'
...
-- To uderstand the transaction we follow  -- commit  ,  rollback and savepoint
...
DELIMITER //
CREATE PROCEDURE CalculateEMI(IN loan_id INT)
BEGIN
    SELECT loan_amount, interest_rate, tenure, 
    (loan_amount * interest_rate / 100) / tenure AS EMI 
